...
class LSTMModule(nn.Module):
    ...
    def forward(self, input_ids: list[int] | Tensor, prev_state: Tuple[Tensor, Tensor] = None) -> Tuple[Tensor, Tuple[Tensor, Tensor]]:
    ...
    def predict_next_token(self, temperature: float, input_ids: list[int], state: Tuple[Tensor, Tensor] = None) -> Tuple[Number, Tuple[Tensor, Tensor]]:
        self.eval()
        with torch.no_grad():
            logits, new_state = self.forward(input_ids=input_ids, prev_state=state)
    ...
    def generate(self, prompt: str, max_output: int, eos_token_ids: list[int], temperature: float=0.1, device: str='cpu') -> str:
        """
